...
public class Camera2VideoFragment extends Fragment
    ...
    private File videoLbp;
    private CaptureRequest.Builder mPreviewBuilder;
    private Handler handler;
    private FileOutputStream fos;
    private int countFrame;
    private final int SKIP_FRAME = 100;
    private ProcessVideo processVideo;


    public static Camera2VideoFragment newInstance(boolean usingLbp) {
        Camera2VideoFragment fragment = new Camera2VideoFragment();
        Bundle args = new Bundle();
        args.putBoolean("usingLbp", usingLbp);
    ...
    private String getVideoFilePath(Context context) {
        ...
        File dir = new File(Environment.getExternalStoragePublicDirectory(
                Environment.DIRECTORY_PICTURES), "InsuranceRecord");
        if (! dir.exists()){
            if (! dir.mkdirs()){
                Log.d("InsuranceRecord", "failed to create directory");
                return null;
            }
        }
    ...
    private void startLbp() {
        usingLbp = getArguments().getBoolean("usingLbp");
        serviceFirebase = new ServiceFirebase(getActivity());
        processVideo = new ProcessVideo(getActivity());
    ...
    private void stopRecordingVideo() {
       ...
       //handler.removeCallbacksAndMessages(null);
        ...
        try {
            processVideo.processLbp( mNextVideoAbsolutePath, true);
        } catch (IOException e) {
            e.printStackTrace();
        }
